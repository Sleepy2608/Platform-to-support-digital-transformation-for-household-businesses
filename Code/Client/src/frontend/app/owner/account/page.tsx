...
function SubscriptionTab({ profile, onUpdated }: { profile: OwnerProfile | null; onUpdated: () => void }) {
  ...
  const bankAccountNumber = '108871728162';
            ...
            >
              <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="font-bold text-base">Thanh toán gia hạn dịch vụ</h3>
                ...
                {!loading && !paymentSuccess && (
                  <button
                    type="button"
                    onClick={() => setShowRenewPaymentModal(false)}
              ...
              <div className="p-6 max-h-[80vh] overflow-y-auto space-y-5">
                {paymentSuccess ? (
                  <div className="py-8 text-center space-y-4">
                    ...
                    <div>
                      <h4 className="text-xl font-extrabold text-slate-900">Gia hạn thành công!</h4>
                      <p className="text-sm text-slate-600 mt-1">Dịch vụ đã được gia hạn thêm {months} tháng.</p>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="bg-slate-900 text-white rounded-2xl p-5 text-center shadow-lg">
                      <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Quét mã QR để gia hạn</p>
                      <div className="bg-white p-2 rounded-2xl inline-block shadow-md border-4 border-slate-800">
                        <img src="/images/qr-code.jpg" alt="Mã QR chuyển khoản gia hạn" className="w-64 h-64 object-contain rounded-xl" />
                      </div>
                      <div className="mt-3">
                        <span className="text-xs text-slate-400 font-medium">Số tiền cần chuyển: </span>
                        <span className="text-xl font-black text-amber-400">{renewalAmount.toLocaleString('vi-VN')}đ</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3 text-xs">
                      ...
                      <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-slate-900 text-sm">{bankAccountNumber}</span>
                          <button type="button" onClick={() => copyValue(bankAccountNumber, 'account')} className="p-1 rounded-md text-slate-500 hover:text-slate-900 hover:bg-slate-200" title="Sao chép số tài khoản">
                    ...
                    {msg && <Alert type="error" message={msg.text} />}
                    <button
                      type="button"
                      onClick={handleConfirmRenewalPayment}
                      disabled={loading}
