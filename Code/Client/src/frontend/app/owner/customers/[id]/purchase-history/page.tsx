...
import {
  ...
  TrendingUp, AlertTriangle, CalendarDays, X, ShoppingBag, History,
} from 'lucide-react';
import Link from 'next/link';
import { useParams } from 'next/navigation';
import { apiClient } from '@/app/lib/apiClient';
import CustomerOrderDetailModal from '@/app/components/CustomerOrderDetailModal';
import CustomerDebtCard from '@/app/components/payment/CustomerDebtCard';
...
/* ─── Page ───────────────────────────────────────────────────────────── */
export default function OwnerPurchaseHistoryPage() {
  ...
  const [activeTab, setActiveTab] = useState<'ORDERS' | 'DEBT_LEDGER'>('ORDERS');
  ...
  const reloadCustomerSummary = useCallback(async () => {
    try {
      const sum = await apiClient.get<PurchaseSummary>(`/api/customers/${customerId}/purchase-summary`);
      setSummary(sum);
    } catch {
      // ignore
    }
  ...
  return (
    <div className="min-h-screen p-5 sm:p-8 lg:p-10">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        {/* Customer Debt Card - SSOT 4 metrics */}
        <CustomerDebtCard customerId={customerId} refreshTrigger={refreshTrigger} />
        ...
        <div className="flex border-b border-slate-200 gap-6 text-sm font-bold">
          ...
          >
            ...
            <span>Sổ giao dịch công nợ & thanh toán</span>
          </button>
        </div>
        ...
        {activeTab === 'ORDERS' ? (
          <>
            ...
            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 px-5 py-3.5 flex items-center justify-between">
                <span className="text-sm font-bold text-slate-700">Danh sách đơn mua hàng</span>
                <button
          ...
          /* Tab 2: Sổ giao dịch công nợ & thanh toán */
          <PaymentHistoryList customerId={customerId} refreshTrigger={refreshTrigger} />
        )}
      </div>
