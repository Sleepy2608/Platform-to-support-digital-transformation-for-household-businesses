...
import {
  ...
  PackageX, RefreshCw, Search, Warehouse, Scale, X,
} from 'lucide-react';
...
export default function CurrentStockBalanceDashboard({
  ...
  const mainContent = (
    <div className="space-y-6">
      {/* Page Title Header (only when not hidden) */}
      {!hideHeader && (
          ...
          <div className="flex items-center gap-3 self-start sm:self-auto">
            ...
            {canAdjust && (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-purple-600 px-4 py-2.5 text-xs sm:text-sm font-bold text-white shadow-sm hover:bg-purple-700 transition cursor-pointer"
              >
          ...
          <div className="flex items-center gap-2">
            {hideHeader && canAdjust && (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-purple-600 px-4 py-2.5 text-xs sm:text-sm font-bold text-white shadow-sm hover:bg-purple-700 transition cursor-pointer shrink-0"
              >
                <Scale className="h-4 w-4" />
                Điều chỉnh tồn kho
              </button>
            ...
            {hideHeader && (
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs sm:text-sm font-bold text-slate-700 shadow-2xs hover:bg-slate-50 transition cursor-pointer disabled:opacity-50 shrink-0"
              >
                <RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} />
              </button>
            )}
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/60 text-xs font-semibold uppercase tracking-wider text-slate-500">
                    ...
                    {canAdjust && <th className="px-5 py-3.5 text-right">Thao tác</th>}
                  </tr>
                </thead>
                      ...
                      {canAdjust && (
                        <td className="px-5 py-3.5 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-purple-200 bg-purple-50 px-2.5 py-1 text-xs font-bold text-purple-700 hover:bg-purple-100 transition cursor-pointer"
                          >
